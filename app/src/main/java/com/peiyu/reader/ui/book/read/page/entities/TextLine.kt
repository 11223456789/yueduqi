...
import com.peiyu.reader.help.PaintPool
...
import com.peiyu.reader.ui.book.read.page.entities.TextPage.Companion.emptyTextPage
import com.peiyu.reader.ui.book.read.page.entities.column.BaseColumn
...
import com.peiyu.reader.utils.canvasrecorder.recordIfNeededThenDraw
...
data class TextLine(
    ...
    fun isVisible(relativeOffset: Float): Boolean {
        ...
        val visible = when {
            ...
            top <= visibleTop && bottom >= visibleBottom -> true
            // 上方第一行部分可�?            top < visibleTop && bottom > visibleTop && bottom < visibleBottom -> {
                if (isImage) {
                    true
                } else {
            ...
            // 下方第一行部分可�?            top > visibleTop && top < visibleBottom && bottom > visibleBottom -> {
                if (isImage) {
                    true
                } else {
    ...
    private fun drawTextLine(view: ContentTextView, canvas: Canvas) {
        ...
        // 墨水屏模式下的朗读和搜索下划�?        if (AppConfig.isEInkMode && (isReadAloud || searchResultColumnCount > 0)) {
