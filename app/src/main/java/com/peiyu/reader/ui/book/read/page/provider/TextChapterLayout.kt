...
import com.peiyu.reader.utils.getTextWidthsCompat
...
class TextChapterLayout(
    ...
    private suspend fun getTextChapter(
        ...
        if (titleMode != 2 || bookChapter.isVolume || contents.isEmpty()) {
            ...
            // 如果是单图模式且当前页有内容，强制分�?            if (isSingleImageStyle && pendingTextPage.lines.isNotEmpty() && contents.isNotEmpty()) {
        ...
        contents.forEach { content ->
            ...
            if (isTextImageStyle) {
                //图片样式为文字嵌入类�?                var text = content.replace(ChapterProvider.srcReplaceChar, "�?)
    ...
    private suspend fun setTypeImage(
        ...
        if (size.width > 0 && size.height > 0) {
            ...
            stringBuilder.append(" ") // 确保翻页时索引计算正�?            pendingTextPage.addLine(textLine)
        }
    ...
    private suspend fun setTypeText(
        ...
        durY = when {
            //标题y轴居�?            emptyContent && textPages.isEmpty() -> {
                val textPage = pendingTextPage
