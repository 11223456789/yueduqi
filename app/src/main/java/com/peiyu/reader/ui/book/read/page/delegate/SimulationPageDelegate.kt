...
import com.peiyu.reader.utils.screenshot
...
class SimulationPageDelegate(readView: ReadView) : HorizontalPageDelegate(readView) {
    //不让x,y�?,否则在点计算时会有问�?    private var mTouchX = 0.1f
    ...
    // 贝塞尔曲线顶�?    private val mBezierVertex1 = PointF()
    ...
    // 贝塞尔曲线顶�?    private val mBezierVertex2 = PointF()
    ...
    // 背面颜色�?    private var mBackShadowColors: IntArray

    // 前面颜色�?    private var mFrontShadowColors: IntArray
    ...
    override fun onAnimStart(animationSpeed: Int) {
        var dx: Float
        val dy: Float
        // dy 垂直方向滑动的距离，负值会使滚动向上滚�?        if (isCancel) {
    ...
    private fun drawNextPageAreaAndShadow(
        ...
        if (mIsRtOrLb) { //左下及右�?            leftX = mBezierStart1.x.toInt()
        ...
        ) //左上及右下角的xy坐标�?构成一个矩�?        mBackShadowDrawable.draw(canvas)
        canvas.restore()
    }
    ...
    private fun calcPoints() {
        ...
        // 固定左边上下两个�?        if (mTouchX > 0 && mTouchX < viewWidth) {
