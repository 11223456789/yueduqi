...
import com.peiyu.reader.ui.book.read.page.entities.column.ButtonColumn
...
class ContentTextView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    fun setContent(textPage: TextPage) {
        ...
        // 非滑动翻页动画需要同步重绘，不然翻页可能会出现闪�?        if (isScroll) {
            postInvalidate()
        } else {
            invalidate()
        }
     ...
     * pageOffset 范围 0 ~ -textPage.height 大于0为上一页，小于-textPage.height为下一�?     * 以内容显示区域顶端为界，pageOffset的绝对值为textPage上方的高�?     * pageOffset + textPage.height �?textPage 下方的高�?     */
