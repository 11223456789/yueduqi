...
import com.peiyu.reader.ui.widget.image.photo.Info
import com.peiyu.reader.ui.widget.image.photo.OnRotateListener
import com.peiyu.reader.ui.widget.image.photo.RotateGestureDetector
...
class PhotoView @JvmOverloads constructor(
    ...
    // 当前是否处于放大状�?    private var isZoonUp = false
    ...
    /**
     * 匹配两个Rect的共同部分输出到out，若无共同部分则输出0�?�?�?
     */
     ...
     * (等待的时间默�?00毫秒，可以通过setMaxAnimFromWaiteTime(int)设置最大等待时�?
     */
    ...
    fun animaTo(
        ...
        if (isInit) {
            ...
            // 将图片旋转回正常位置，用以计�?            mAnimMatrix.postRotate(-mDegrees, mScaleCenter.x, mScaleCenter.y)
            mAnimMatrix.mapRect(mImgRect, mBaseRect)
