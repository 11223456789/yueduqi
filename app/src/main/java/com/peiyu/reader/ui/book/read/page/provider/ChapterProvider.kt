...
import com.peiyu.reader.ui.book.read.page.entities.column.ImageColumn
import com.peiyu.reader.ui.book.read.page.entities.column.ReviewColumn
import com.peiyu.reader.ui.book.read.page.entities.column.TextColumn
import com.peiyu.reader.utils.RealPathUtil
import com.peiyu.reader.utils.buildMainHandler
import com.peiyu.reader.utils.dpToPx
import com.peiyu.reader.utils.fastSum
import com.peiyu.reader.utils.isContentScheme
import com.peiyu.reader.utils.isPad
import com.peiyu.reader.utils.postEvent
import com.peiyu.reader.utils.spToPx
import com.peiyu.reader.utils.splitNotBlank
import com.peiyu.reader.utils.textHeight
...
object ChapterProvider {
    ...
    suspend fun getTextChapter(
        ...
        if (ReadBookConfig.titleMode != 2 || bookChapter.isVolume) {
            //标题非隐�?            displayTitle.splitNotBlank("\n").forEach { text ->
                setTypeText(
                    book, absStartX, durY,
        ...
        contents.forEach { content ->
            if (book.getImageStyle().equals(Book.imgStyleText, true)) {
                //图片样式为文字嵌入类�?                var text = content.replace(srcReplaceChar, "�?)
    ...
    private suspend fun setTypeImage(
        ...
        if (size.width > 0 && size.height > 0) {
            ...
            when (imageStyle?.uppercase(Locale.ROOT)) {
                ...
                Book.imgStyleSingle -> {
                    ...
                    if (durY > 0f) {
                        ...
                        // 双页�?durY 不正确，可能会小于实际高�?                        if (textPage.height < durY) {
                            textPage.height = durY
                        }
                        durY = 0f
                    }
            ...
            stringBuilder.append(" ") // 确保翻页时索引计算正�?            textPages.last().addLine(textLine)
        }
    ...
    private suspend fun setTypeText(
        ...
        var durY = when {
            //标题y轴居�?            emptyContent && textPages.size == 1 -> {
                val textPage = textPages.last()
        ...
        for (lineIndex in 0 until layout.lineCount) {
            ...
            if (durY + textHeight > visibleHeight) {
                ...
                if (doublePage && absStartX < viewWidth / 2) {
                    ...
                    //当前页面结束,设置各种�?                    if (textPage.leftLineSize == 0) {
                        textPage.leftLineSize = textPage.lineSize
                    }
                    textPage.text = stringBuilder.toString()
                    //新建页面
