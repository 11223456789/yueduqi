...
import com.peiyu.reader.help.book.updateTo
import com.peiyu.reader.help.coroutine.Coroutine
import com.peiyu.reader.lib.webdav.ObjectNotFoundException
...
class BookInfoViewModel(application: Application) : BaseViewModel(application) {
    ...
    fun initData(intent: Intent) {
        execute {
            ...
            throw NoStackTraceException("未找到书�?)
        }.onError {
            ...
            WebBook.getBookInfo(scope, bookSource, book, canReName = canReName)
                .onSuccess(IO) {
                    ...
                    if (!inBookshelf && dbBook != null && !dbBook.isNotShelf && dbBook.origin == book.origin) {
                        /**
                         * book 来自搜索�?inBookshelf == false)，搜索的书名不存在于书架，但是加载详情后，书名更新，存在同名书籍
                         * 此时 book 的数据会与数据库中的不同，需要更�?#3652 #4619
                         * book 加载详情后虽然书名作者相同，但是又可能不是数据库�?书源不同)的那本书 #3149
                         */
                        dbBook.updateTo(it)
                        inBookshelf = true
                    }
            ...
            WebBook.getChapterList(scope, bookSource, book, runPreUpdateJs)
                .onSuccess(IO) {
                    if (inBookshelf) {
                        ...
                        /**
                         * runPreUpdateJs 有可能会修改 book �?bookUrl
                         */
    ...
    data class WebFile(
        ...
        // txt epub umd pdf等文�?        val isSupported: Boolean = AppPattern.bookFileRegex.matches(name)
