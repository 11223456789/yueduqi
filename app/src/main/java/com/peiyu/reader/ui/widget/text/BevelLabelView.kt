...
class BevelLabelView @JvmOverloads constructor(
    ...
    private var mRotate = 45 //因为默认模式�?，所以这时是45�?
    ...
    private fun drawBackgroundText(canvas: Canvas) {
        ...
        val baseLineY = (-(mPaint.descent() + mPaint.ascent())).toInt() / 2 //基线中间点的y轴计算公�?        canvas.drawText(mText, 0f, baseLineY.toFloat(), mPaint)
    }
    ...
    //左上角铺�?    private fun getLeftTopFill() {
        if (mCorner != 0) {
            path.addRoundRect(
                0f,
                0f,
    ...
    //左下角铺�?    private fun getLeftBottomFill() {
        if (mCorner != 0) {
            path.addRoundRect(
                0f,
                (mHeight / 2).toFloat(),
    ...
    //右上角铺�?    private fun getRightTopFill() {
        if (mCorner != 0) {
            path.addRoundRect(
                (mWidth / 2).toFloat(),
                0f,
    ...
    //右下角铺�?    private fun getRightBottomFill() {
        if (mCorner != 0) {
            path.addRoundRect(
                (mWidth / 2).toFloat(),
                (mHeight / 2).toFloat(),
