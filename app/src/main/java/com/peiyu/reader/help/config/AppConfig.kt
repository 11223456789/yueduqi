package com.peiyu.reader.help.config
...
object AppConfig : SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    // 保存 自定义导出章节模�?文件名js表达�?    var episodeExportFileName: String?
    ...
    // 添加本地选择的目�?    var importBookPath: String?
    ...
    fun detectClickArea() {
        if (clickActionTL * clickActionTC * clickActionTR
            ...
            * clickActionBL * clickActionBC * clickActionBR != 0
            ...
            appCtx.toastOnUi("当前没有配置菜单区域,自动恢复中间区域为菜�?")
        }
    }

    //跳转到漫画界面不使用富文本模�?    val showMangaUi: Boolean
    ...
    //漫画预加载数�?    var mangaPreDownloadNum
