...
object RuleComplete {
    // 需要补�?    private val needComplete = Regex(
        """(?<!(@|/|^|[|%&]{2})(attr|text|ownText|textNodes|href|content|html|alt|all|value|src)(\(\))?)(?<seq>\&{2}|%%|\|{2}|$)"""
    )

    // 不能补全 存在js/json/{{xx}}的复杂情�?    private val notComplete = Regex("""^:|^##|\{\{|@js:|<js>|@Json:|\$\.""")

    // 修正从图片获取信�?    private val fixImgInfo =
     ...
     * @author 希弥
     * @return 补全后的规则 �?原规�?     * @param rules 需要补全的规则
