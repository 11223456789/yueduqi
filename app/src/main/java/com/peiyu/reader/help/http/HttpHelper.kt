...
import com.peiyu.reader.help.glide.progress.ProgressManager.LISTENER
import com.peiyu.reader.help.glide.progress.ProgressResponseBody
...
val cookieJar by lazy {
    object : CookieJar {
        ...
        override fun saveFromResponse(url: HttpUrl, cookies: List<Cookie>) {
            ...
            //临时保存 书源启用cookie选项再添加到数据�?            val cookieBuilder = StringBuilder()
...
fun getProxyClient(proxy: String? = null): OkHttpClient {
    ...
    var password = ""       //代理服务器验证密�?    val type = if (group.groupValues[1] == "http") "http" else "socks"
    ...
    if (host != "") {
        ...
        if (username != "" && password != "") {
            builder.proxyAuthenticator { _, response -> //设置代理服务器账号密�?                val credential: String = Credentials.basic(username, password)
