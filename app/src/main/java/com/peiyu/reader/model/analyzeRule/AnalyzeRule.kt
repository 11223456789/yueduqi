...
import com.peiyu.reader.utils.getOrPutLimit
...
class AnalyzeRule(
        ...
        if (content != null && ruleList.isNotEmpty()) {
            ...
            if (result is NativeObject) {
                ...
                result = if (sourceRule.getParamSize() > 1) {
                    ...
                    // 键值直接访�?                    result[sourceRule.rule]?.toString()
                }?.let {
            ...
            GSON.fromJsonObject<Map<String, String>>(putJsonStr)
                ...
                ?.let {
                    if (!loggedNonStandardJSON) {
                        Debug.log("≡@put 规则 JSON 格式不规范，请改为规范格�?)
    ...
    fun splitSourceRule(ruleStr: String?, allInOne: Boolean = false): List<SourceRule> {
        ...
        var start = 0
        //仅首字符�?时为AllInOne，其�?与伪类选择器冲突，建议改成?更合�?        if (allInOne && ruleStr.startsWith(":")) {
            mMode = Mode.Regex
            isRegex = true
            start = 1
        } else if (isRegex) {
    ...
    inner class SourceRule internal constructor(
        ...
        init {
            rule = when {
                ...
                ruleStr.startsWith("/") -> {//XPath特征很明�?无需配置单独的识别标�?                    mode = Mode.XPath
                    ruleStr
                }
        ...
        fun makeUpRule(result: Any?) {
            ...
            //分离正则表达�?            val ruleStrS = rule.split("##")
     ...
     */
    fun reGetBook() {
        if (!preUpdateJs) throw NoStackTraceException("只能�?preUpdateJs 中调�?)
