...
object Debug {
    ...
    fun startChecking(source: BookSource) {
        ...
        debugMessageMap[source.bookSourceUrl] = "${debugTimeFormat.format(Date(0))} 开始校�?
    }
    ...
    suspend fun startDebug(scope: CoroutineScope, rssSource: RssSource) {
        ...
        Rss.getArticles(scope, sort.first, sort.second, rssSource, 1)
            .onSuccess {
                if (it.first.isEmpty()) {
                    log(debugSource, "⇒列表页解析成功，为�?)
                    log(debugSource, "︽解析完�?, state = 1000)
                } else {
    ...
    fun startDebug(scope: CoroutineScope, bookSource: BookSource, key: String) {
        ...
        when {
            ...
            key.startsWith("++") -> {
                ...
                book.origin = bookSource.bookSourceUrl
                book.tocUrl = url
                log(bookSource.bookSourceUrl, "⇒开始访目录�?$url")
            ...
            key.startsWith("--") -> {
                ...
                val book = Book()
                book.origin = bookSource.bookSourceUrl
                log(bookSource.bookSourceUrl, "⇒开始访正文�?$url")
