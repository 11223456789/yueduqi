...
import com.peiyu.reader.help.book.removeAllBookType
...
object WebBook {
    ...
    suspend fun getContentAwait(
        ...
        if (bookSource.getContentRule().content.isNullOrEmpty()) {
            Debug.log(bookSource.bookSourceUrl, "⇒正文规则为�?使用章节链接:${bookChapter.url}")
    ...
    fun preciseSearch(
        ...
        return Coroutine.async(scope, context, semaphore = semaphore) {
            ...
            throw NoStackTraceException("没有搜索�?$name>$author")
        }
    }
    ...
    private fun checkRedirect(bookSource: BookSource, response: StrResponse) {
        response.raw.priorResponse?.let {
            if (it.isRedirect) {
                Debug.log(bookSource.bookSourceUrl, "≡检测到重定�?${it.code})")
