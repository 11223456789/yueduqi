...
public class QueryTTF {
    ...
    private static class NameRecord {
        public int platformID;           // 平台标识�?0:Unicode, 1:Mac, 2:ISO, 3:Windows, 4:Custom>
        public int encodingID;           // 编码标识�?        public int languageID;           // 语言标识�?        public int nameID;               // 名称标识�?        public int length;               // 名称字符串的长度
    ...
    private static class HeadLayout {
        ...
        /**
         * int16
         * <p> 0 表示短偏�?(Offset16)�? 表示长偏�?(Offset32)�?         */
    ...
    private static class CmapFormat {
         ...
         * uint16[256]
         * <p> �?Format=2
         * <p> 将高字节映射�?subHeaders 的数组：值为 subHeader 索引x8
         */
         ...
         * uint16   segCount x2
         * <p> �?Format=4
         * <p> seg段计数乘�?2。这是因为每个段用两个字节表示，所以这个值是实际段数的两倍�?         */
         ...
         * uint16[segCount]
         * <p> �?Format=4
         * <p> 每个段的结束字符码，最后一个是 0xFFFF，表�?Unicode 范围的结束�?         */
        public int[] endCode;
        /**
         * uint16
         * <p> �?Format=4
         * <p> 固定设置�?0，用于填充保留位以保持数据对齐�?         */
         ...
         * int16[segCount]
         * <p> �?Format=4
         * <p> 用于计算字形索引的偏移值。该值被加到�?startCode �?endCode 的所有字符码上，得到相应的字形索引�?         */
        public int[] idDelta;
        /**
         * uint16[segCount]
         * <p> �?Format=4
         * <p> 偏移�?glyphIdArray 中的起始位置，如果没有额外的字形索引映射，则�?0�?         */
         ...
         * uint16
         * <p> �?Format=6
         * <p> 子范围中字符代码的数量。这表示�?firstCode 开始，连续多少个字符代码被包含
         */
    ...
    private static class GlyphTableBySimple {
        ...
        /**
         * uint8[variable]
         * <p> bit0: 该点位于曲线�?         * <p> bit1: < 1:xCoordinate为uint8 >
         * <p> bit2: < 1:yCoordinate为uint8 >
         * <p> bit3: < 1:下一个uint8为此条目之后插入的附加逻辑标志条目的数�?>
         * <p> bit4: < bit1=1时表示符号[1.�?0.负]; bit1=0时[1.x坐标重复一�?0.x坐标读为int16] >
         * <p> bit5: < bit2=1时表示符号[1.�?0.负]; bit2=0时[1.y坐标重复一�?0.y坐标读为int16] >
         * <p> bit6: 字形描述中的轮廓可能会重�?         * <p> bit7: 保留�?无意�?         */
        int[] flags;
        /**
    ...
    private static class GlyphTableComponent {
        /**
         * uint16
         * <p> bit0: < 1:argument�?6bit�?:argument�?bit >
         * <p> bit1: < 1:argument是有符号值，0:argument是无符号�?>
         * <p> bit3: 该组件有一个缩放比例，否则比例�?.0
         * <p> bit5: 表示在此字形之后还有字形
         */
        int flags;
        /**
        ...
        /**
         * uint16
         * <p> 值类型为 F2DOT14 的组件缩放X比例�?         */
        float xScale;
        /**
         * uint16
         * <p> 值类型为 F2DOT14 �?x2变换矩阵01�?         */
        float scale01;
        /**
         * uint16
         * <p> 值类型为 F2DOT14 �?x2变换矩阵10�?         */
        float scale10;
        /**
         * uint16
         * <p> 值类型为 F2DOT14 的组件缩放Y比例�?         */
        float yScale;
    }
    ...
    private static class BufferReader {
        ...
        public BufferReader(byte[] buffer, int index) {
            ...
            this.byteBuffer.order(ByteOrder.BIG_ENDIAN); // 设置为大端模�?            this.byteBuffer.position(index); // 设置起始索引
        }
    ...
    /**
     * glyfId到glyphData的索�?     * <p> 根据定义，索引零指向“丢失的字符”�?     * <p> loca.length = maxp.numGlyphs + 1;
     */
    private int[] loca;
    ...
    private void readLocaTable(byte[] buffer) {
        ...
        if (head.indexToLocFormat == 0) {
            ...
            // 当loca表数据长度为Uint16�?需要翻�?            for (var i = 0; i < loca.length; i++) loca[i] *= 2;
        } else {
        ...
        for (var formatTable : Cmap.records) {
            ...
            switch (f.format) {
                case 0: {
                    ...
                    // 记录 unicode->glyphId 映射�?                    int unicodeInclusive = 0;
                    int unicodeExclusive = f.glyphIdArray.length;
                    for (; unicodeInclusive < unicodeExclusive; unicodeInclusive++) {
                        if (f.glyphIdArray[unicodeInclusive] == 0) continue; // 排除轮廓索引�?的Unicode
                ...
                case 4: {
                    ...
                    // 一个包含字形索引的数组，其长度是任意的，取决于映射的复杂性和字体中的字符数量�?                    int glyphIdArrayLength = (f.length - 16 - (segCount * 8)) / 2;
                    f.glyphIdArray = reader.ReadUInt16Array(glyphIdArrayLength);

                    // 记录 unicode->glyphId 映射�?                    for (int segmentIndex = 0; segmentIndex < segCount; segmentIndex++) {
                        ...
                        for (int unicode = unicodeInclusive; unicode <= unicodeExclusive; unicode++) {
                            ...
                            if (glyphId == 0) continue; // 排除轮廓索引�?的Unicode
                ...
                case 6: {
                    ...
                    // 范围内字符代码的字形索引值数组�?                    f.glyphIdArray = reader.ReadUInt16Array(f.entryCount);

                    // 记录 unicode->glyphId 映射�?                    int unicodeIndex = f.firstCode;
    ...
    private void readGlyfTable(byte[] buffer) {
        ...
        for (int index = 0; index < glyfCount; index++) {
            ...
            if (glyph.numberOfContours > maxp.maxContours) continue; // 如果字形轮廓数大于非复合字形中包含的最大轮廓数，则说明该字形无效�?            glyph.xMin = reader.ReadInt16();
            glyph.yMin = reader.ReadInt16();
            glyph.xMax = reader.ReadInt16();
            glyph.yMax = reader.ReadInt16();

            // 轮廓数为0时，不需要解析轮廓数�?            if (glyph.numberOfContours == 0) continue;
            // 读Glyph轮廓数据
            if (glyph.numberOfContours > 0) {
                // 简单轮�?                glyph.glyphSimple = new GlyphTableBySimple();
                ...
                // 获取轮廓点描述标�?                glyph.glyphSimple.flags = new int[flagLength];
                ...
                // 获取轮廓点描述x轴相对�?                glyph.glyphSimple.xCoordinates = new int[flagLength];
                for (int n = 0; n < flagLength; ++n) {
                    switch (glyph.glyphSimple.flags[n] & 0x12) {
                        ...
                        case 0x12:
                            ...
                            break;
                        case 0x10:
                            glyph.glyphSimple.xCoordinates[n] = 0;  // 点位数据重复上一次数据，那么相对数据变化量就�?
                            break;
                        case 0x00:
                ...
                // 获取轮廓点描述y轴相对�?                glyph.glyphSimple.yCoordinates = new int[flagLength];
                for (int n = 0; n < flagLength; ++n) {
                    switch (glyph.glyphSimple.flags[n] & 0x24) {
                        ...
                        case 0x24:
                            ...
                            break;
                        case 0x20:
                            glyph.glyphSimple.yCoordinates[n] = 0;  // 点位数据重复上一次数据，那么相对数据变化量就�?
                            break;
                        case 0x00:
                ...
                while (true) {
                    ...
                    switch (glyphTableComponent.flags & 0b11001000) {
                        case 0b00001000:
                            ...
                            break;
                        case 0b01000000:
                            // 有X和Y的独立比�?                            glyphTableComponent.xScale = ((float) reader.ReadUInt16()) / 16384.0f;
    ...
    public String getGlyfById(int glyfId) {
        ...
        String glyphString;
        if (glyph.numberOfContours >= 0) {
            // 简单字�?            int dataCount = glyph.glyphSimple.flags.length;
    ...
    /**
     * 构造函�?     *
     * @param buffer 传入TTF字体二进制数�?     */
    public QueryTTF(final byte[] buffer) {
        var fontReader = new BufferReader(buffer, 0);
//        Log.i("QueryTTF", "读文件头"); // 获取文件�?        fileHeader.sfntVersion = fontReader.ReadUInt32();
...
//        Log.i("QueryTTF", "解析�?name"); // 字体信息,包含版权、名称、作者等...
        readNameTable(buffer);
//        Log.i("QueryTTF", "解析�?head"); // 获取 head.indexToLocFormat
        readHeadTable(buffer);
//        Log.i("QueryTTF", "解析�?cmap"); // Unicode编码->轮廓索引 对照�?        readCmapTable(buffer);
//        Log.i("QueryTTF", "解析�?loca"); // 轮廓数据偏移地址�?        readLocaTable(buffer);
//        Log.i("QueryTTF", "解析�?maxp"); // 获取 maxp.numGlyphs 字体轮廓数量
        readMaxpTable(buffer);
//        Log.i("QueryTTF", "解析�?glyf"); // 字体轮廓数据�?需要解析loca,maxp表后计算
        readGlyfTable(buffer);
//        Log.i("QueryTTF", "建立Unicode&Glyph映射�?);
...
//        Log.i("QueryTTF", "字体处理完成");
    ...
    /**
     * 使用 Unicode 值获查询廓索�?     *
     * @param unicode 传入 Unicode �?     * @return 轮廓索引
     */
    public int getGlyfIdByUnicode(int unicode) {
        var result = unicodeToGlyphId.get(unicode);
        if (result == null) return 0; // 如果找不到Unicode对应的轮廓索引，就返回默认�?
        return result;
    }

    /**
     * 使用 Unicode 值查询轮廓数�?     *
     * @param unicode 传入 Unicode �?     * @return 轮廓数据
     */
    ...
    public int getUnicodeByGlyf(String glyph) {
        ...
        if (result == null) return 0; // 如果轮廓数据找不到对应的Unicode，就返回默认�?
        return result;
    }
     ...
     * Unicode 空白字符判断
     *
     * @param unicode 字符�?Unicode �?     * @return true:是空白字�? false:非空白字�?     */
    public boolean isBlankUnicode(int unicode) {
        return switch (unicode) {
            case 0x0009,    // 水平制表�?(Horizontal Tab)
                    0x0020,    // 空格 (Space)
                    0x00A0,    // 不中断空�?(No-Break Space)
                    0x2002,    // En空格 (En Space)
                    0x2003,    // Em空格 (Em Space)
                    0x2007,    // 刚性空�?(Figure Space)
                    0x200A,    // 发音修饰字母的连字符 (Hair Space)
                    0x200B,    // 零宽空格 (Zero Width Space)
                    0x200C,    // 零宽不连�?(Zero Width Non-Joiner)
                    0x200D,    // 零宽连字 (Zero Width Joiner)
                    0x202F,    // 狭窄不中断空�?(Narrow No-Break Space)
