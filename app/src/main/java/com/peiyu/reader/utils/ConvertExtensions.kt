...
object ConvertUtils {
    ...
    fun toBitmap(bytes: ByteArray, width: Int = -1, height: Int = -1): Bitmap? {
        ...
        if (bytes.isNotEmpty()) {
            kotlin.runCatching {
                ...
                // 设置让解码器以最佳方式解�?                options.inPreferredConfig = null
                if (width > 0 && height > 0) {
    ...
    fun formatFileSize(length: Long): String {
        ...
        //计算单位的，原理是利用lg,公式�?lg(1024^n) = nlg(1024)，最�?nlg(1024)/lg(1024) = n�?        val digitGroups = (log10(length.toDouble()) / log10(1024.0)).toInt()
